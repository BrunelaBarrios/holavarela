  ...
  return <main className="min-h-screen bg-[#f5f7f8] text-slate-900">
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:py-12">
      ...
      <section id="builder" className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        ...
        {step === 3 ? <PaymentStep paymentLink={appliedPromo==="half"?halfPaymentLink:paymentLink} price={appliedPromo==="half"?100:200} operation={operation} setOperation={setOperation} receipt={receipt} setReceipt={setReceipt} status={paymentStatus} submit={submitPayment} busy={busy} refresh={refreshStatus} promoCode={promoCode} setPromoCode={setPromoCode} applyPromo={applyPromo} appliedPromo={appliedPromo}/> : null}
...
function PaymentStep({paymentLink,price,operation,setOperation,receipt,setReceipt,status,submit,busy,refresh,promoCode,setPromoCode,applyPromo,appliedPromo}:any) { return <div className="mx-auto max-w-2xl py-4"><SectionHeading eyebrow="Paso 4" title="Tu currículum está pronto" text="Para descargarlo en formato PDF e imagen, realizá un único pago de:"/><div className="my-7 rounded-3xl bg-[#082f49] p-7 text-center text-white"><p className="text-sm font-bold uppercase tracking-widest text-cyan-300">Total</p><p className="mt-2 text-6xl font-black">{status==="approved"?"$0":`$${price}`}</p>{appliedPromo?<p className="mt-2 font-bold text-emerald-300">{appliedPromo==="free"?"Código gratuito aplicado":"50% de descuento aplicado"}</p>:null}<ul className="mx-auto mt-6 max-w-sm space-y-2 text-left text-sm text-slate-200"><li>✓ Descarga en PDF y como imagen</li><li>✓ Una o varias páginas</li><li>✓ 30 días para editar y volver a descargar</li></ul>{status!=="approved"?<a href={paymentLink} target="_blank" rel="noreferrer" className="mt-7 block rounded-xl bg-cyan-400 px-5 py-4 font-black text-slate-950">Pagar ${price} con Mercado Pago</a>:null}</div><div className="mb-5 rounded-2xl border border-cyan-200 bg-cyan-50 p-5"><h3 className="font-black">¿Tenés un código?</h3><div className="mt-3 flex gap-2"><input value={promoCode} onChange={e=>setPromoCode(e.target.value.toUpperCase())} disabled={!!appliedPromo} placeholder="Ingresá tu código" className="min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-4 py-3 font-bold uppercase"/><button onClick={applyPromo} disabled={busy||!!appliedPromo} className="btn-secondary">Aplicar</button></div></div>{status!=="approved"?<div className="rounded-2xl border p-5"><h3 className="font-black">Confirmá tu pago</h3><p className="mt-1 text-sm text-slate-600">Una vez confirmado, habilitaremos la descarga de tu currículum.</p><div className="mt-5 space-y-4"><Field label="Número de operación" value={operation} onChange={setOperation}/><label className="block text-sm font-bold">Comprobante (opcional)<input type="file" accept="image/*,.pdf" className="mt-2 block w-full text-sm" onChange={e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=()=>setReceipt(String(r.result));r.readAsDataURL(f)}}/></label></div><div className="mt-5 flex flex-wrap gap-3"><button onClick={submit} disabled={busy||status==="pending"} className="btn-primary">{status==="pending"?"Pago enviado":"Enviar comprobante"}</button><button onClick={refresh} className="btn-secondary">Consultar estado</button></div></div>:null}</div> }
