  ...
  return <main className="min-h-screen bg-[#f5f7f8] text-slate-900">
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:py-12">
      ...
      <section id="builder" className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        ...
        {step === 2 ? <div><SectionHeading eyebrow="Paso 3" title="Revisá cómo quedó" text="Podés volver a editar tus datos o cambiar el diseño antes de continuar."/><div className="mt-8 rounded-2xl bg-slate-100 p-2 sm:p-8"><ResponsiveCvPreview previewRef={previewRef} data={data} template={template} watermark/></div></div> : null}
        ...
        {step === 4 ? <div className="py-5 text-center sm:py-8"><div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-emerald-100 text-emerald-700"><Check className="h-8 w-8"/></div><h2 className="mt-5 text-3xl font-black">¡Tu currículum está pronto!</h2><p className="mx-auto mt-3 max-w-xl text-slate-600">Ya podés descargarlo y comenzar a enviarlo a las oportunidades laborales que te interesen.</p><div className="mx-auto mt-8 max-w-[820px] rounded-2xl bg-slate-100 p-2 sm:p-3"><ResponsiveCvPreview previewRef={previewRef} data={data} template={template}/></div><div className="mt-7 grid gap-3 sm:flex sm:flex-wrap sm:justify-center"><button onClick={() => download("pdf")} className="btn-primary w-full sm:w-auto"><Download/>Descargar en PDF</button><button onClick={() => download("image")} className="btn-secondary w-full sm:w-auto"><ImageIcon/>Descargar como imagen</button><button onClick={() => setStep(0)} className="btn-secondary w-full sm:w-auto">Editar mi currículum</button></div></div> : null}
...
function TemplatePicker({value,onChange,data}:{value:string;onChange:(v:any)=>void;data:CvData}) {
  ...
  return <div><SectionHeading eyebrow="Paso 2" title="Elegí el diseño" text="Deslizá para comparar los modelos. Tocá uno para seleccionarlo."/>
    ...
    <div className="mt-5 grid auto-cols-[88%] grid-flow-col gap-4 overflow-x-auto pb-4 pr-4 snap-x snap-mandatory [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:grid-flow-row lg:auto-cols-auto lg:grid-cols-3 lg:overflow-visible lg:pr-0">
      {options.map(([id,title,text])=><button type="button" key={id} onClick={()=>onChange(id)} aria-pressed={value===id} className={`snap-center overflow-hidden rounded-2xl border-2 bg-white p-3 text-left shadow-sm transition ${value===id?"border-cyan-600 ring-4 ring-cyan-100":"border-slate-200"}`}>
        <div className="overflow-hidden rounded-xl bg-slate-100 p-2"><MiniCvPreview data={data} template={id as Template}/></div>
        <div className="p-3"><span className="flex items-center justify-between text-lg font-black">{title}<span className={`grid h-7 w-7 place-items-center rounded-full ${value===id?"bg-cyan-600 text-white":"border border-slate-300 text-transparent"}`}><Check className="h-4 w-4"/></span></span><p className="mt-1 min-h-10 text-sm leading-5 text-slate-600">{text}</p></div>
      </button>)}
    </div>
...
function ResponsiveCvPreview({ data, template, watermark=false, previewRef }: { data:CvData; template:Template; watermark?:boolean; previewRef:React.RefObject<HTMLDivElement|null> }) {
  ...
  const connectRef = useCallback((element:HTMLDivElement|null) => {
    setNode(element)
  ...
  useEffect(() => {
    ...
    const resize=()=>{const next=Math.min(1,container.clientWidth/794);setScale(next);setHeight(node.scrollHeight*next)}
    resize()
    const observer=new ResizeObserver(resize)
    observer.observe(container);observer.observe(node)
    return()=>observer.disconnect()
  },[node])
  return <div ref={containerRef} className="relative w-full overflow-hidden" style={{height}}>
    <div className="absolute left-1/2 top-0" style={{width:794,transform:`translateX(-50%) scale(${scale})`,transformOrigin:"top center"}}>
      <CvPreview ref={connectRef} data={data} template={template} watermark={watermark}/>
    </div>
  </div>
}
function MiniCvPreview({data,template}:{data:CvData;template:Template}) {
  const containerRef=useRef<HTMLDivElement>(null)
  const [scale,setScale]=useState(.3)
  useEffect(()=>{const container=containerRef.current;if(!container)return;const resize=()=>setScale(Math.min(.34,container.clientWidth/794));resize();const observer=new ResizeObserver(resize);observer.observe(container);return()=>observer.disconnect()},[])
  return <div ref={containerRef} className="relative w-full overflow-hidden" style={{height:1123*scale}}><div className="absolute left-1/2 top-0" style={{width:794,transform:`translateX(-50%) scale(${scale})`,transformOrigin:"top center"}}><CvPreview data={data} template={template}/></div></div>
}
...
function PaymentStep({price,status,busy,checkout,promoCode,setPromoCode,applyPromo,appliedPromo}:any) { return <div className="mx-auto max-w-2xl py-4"><SectionHeading eyebrow="Paso 4" title="Tu currículum está pronto" text="Para descargarlo en formato PDF e imagen, realizá un único pago de:"/><div className="my-7 rounded-3xl bg-[#082f49] p-5 text-center text-white sm:p-7"><p className="text-sm font-bold uppercase tracking-widest text-cyan-300">Total</p><p className="mt-2 text-5xl font-black sm:text-6xl">{status==="approved"?"$0":`$${price}`}</p>{appliedPromo?<p className="mt-2 font-bold text-emerald-300">{appliedPromo==="free"?"Código gratuito aplicado":"50% de descuento aplicado"}</p>:null}<ul className="mx-auto mt-6 max-w-sm space-y-2 text-left text-sm text-slate-200"><li>✓ Descarga en PDF y como imagen</li><li>✓ Una o varias páginas</li><li>✓ 30 días para editar y volver a descargar</li></ul>{status!=="approved"?<button onClick={checkout} disabled={busy} className="mt-7 w-full rounded-xl bg-cyan-400 px-4 py-4 font-black text-slate-950 disabled:opacity-60">{busy?"Abriendo Mercado Pago…":`Pagar $${price} con Mercado Pago`}</button>:null}</div><div className="mb-5 rounded-2xl border border-cyan-200 bg-cyan-50 p-4 sm:p-5"><h3 className="font-black">¿Tenés un código?</h3><div className="mt-3 grid gap-2 sm:flex"><input value={promoCode} onChange={e=>setPromoCode(e.target.value.toUpperCase())} disabled={!!appliedPromo} placeholder="Ingresá tu código" className="min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-4 py-3 font-bold uppercase"/><button onClick={applyPromo} disabled={busy||!!appliedPromo} className="btn-secondary w-full sm:w-auto">Aplicar</button></div></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm leading-6 text-emerald-900 sm:p-5"><strong className="block">Confirmación automática</strong>Cuando completes el pago, Mercado Pago te traerá nuevamente a Hola Varela y habilitará la descarga. No necesitás ingresar números de operación ni enviar comprobantes.</div></div> }
