...
import { Check, Download, ExternalLink, FileText, Mail, MapPin, Phone, Search, Trash2, X } from "lucide-react"
...
type Candidate = {
  ...
  telefono:string|null; email:string|null; cv_url:string|null; autoriza_publicacion:boolean
...
const paymentLabels = { draft:"Borrador", pending:"Pago pendiente", approved:"Aprobado", rejected:"Rechazado" }
...
export default function AdminCurriculumsPage() {
  ...
  const updatePayment=async(item:GeneratedCv,estado_pago:GeneratedCv["estado_pago"])=>{
    await fetch("/api/admin/curriculums",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:item.id,estado_pago})});void load()
  }
  ...
  return <div>
    ...
    <div className="mt-5 grid gap-3 rounded-2xl border bg-white p-4 sm:grid-cols-[1fr_220px]">
      ...
      <select value={status} onChange={event=>setStatus(event.target.value)} className="rounded-xl border px-4 py-3">
        ...
        {tab==="received"?candidateStatuses.map(value=><option key={value} value={value}>{value}</option>):<><option value="pending">Pagos pendientes</option><option value="approved">Aprobados</option><option value="rejected">Rechazados</option><option value="draft">Borradores</option></>}
      </select>
    </div>

    {error?<p className="mt-6 rounded-2xl bg-red-50 p-5 text-center font-semibold text-red-700">{error}</p>:loading?<p className="py-16 text-center text-slate-500">Buscando currículums…</p>:visibleCount===0?<div className="mt-6 rounded-2xl bg-white py-16 text-center"><FileText className="mx-auto h-12 w-12 text-slate-300"/><p className="mt-3 text-slate-500">No hay currículums que coincidan.</p></div>:tab==="received"?<ReceivedTable items={candidates} onSelect={setSelected}/>:<GeneratedTable items={generated} onUpdate={updatePayment}/>}
...
function ReceivedTable({items,onSelect}:{items:Candidate[];onSelect:(item:Candidate)=>void}){return <div className="mt-6 overflow-x-auto rounded-2xl border bg-white"><table className="w-full min-w-[1000px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-4">Persona</th><th className="p-4">Puesto buscado</th><th className="p-4">Habilidades</th><th className="p-4">Localidad</th><th className="p-4">Contacto</th><th className="p-4">Estado</th><th className="p-4">CV</th></tr></thead><tbody className="divide-y">{items.map(item=><tr key={item.id} onClick={()=>onSelect(item)} className="cursor-pointer hover:bg-sky-50/60"><td className="p-4"><strong className="block text-slate-950">{item.nombre_completo}</strong><span className="text-xs text-slate-500">{new Date(item.fecha_creacion).toLocaleDateString("es-UY")}</span></td><td className="p-4">{item.puesto_buscado||"General"}</td><td className="max-w-xs p-4"><span className="line-clamp-2">{item.habilidades}</span></td><td className="p-4">{item.localidad}</td><td className="p-4">{item.telefono||item.email||"Sin dato"}</td><td className="p-4"><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold uppercase">{item.estado}</span></td><td className="p-4">{item.cv_url?<a href={item.cv_url} download={`CV-${item.nombre_completo}.${cvExtension(item.cv_url)}`} onClick={event=>event.stopPropagation()} className="inline-flex items-center gap-1 font-bold text-sky-700"><Download className="h-4 w-4"/>Descargar</a>:<span className="text-slate-400">Sin archivo</span>}</td></tr>)}</tbody></table></div>}
function GeneratedTable({items,onUpdate}:{items:GeneratedCv[];onUpdate:(item:GeneratedCv,status:GeneratedCv["estado_pago"])=>void}){return <div className="mt-6 overflow-x-auto rounded-2xl border bg-white"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-4">Persona</th><th className="p-4">Modelo</th><th className="p-4">Pago</th><th className="p-4">Operación</th><th className="p-4">Estado</th><th className="p-4">Creado</th><th className="p-4">Acciones</th></tr></thead><tbody className="divide-y">{items.map(item=><tr key={item.id}><td className="p-4"><strong className="block">{item.nombre}</strong><span className="text-slate-500">{item.email} · {item.telefono}</span></td><td className="p-4 capitalize">{item.modelo}</td><td className="p-4"><strong>${item.monto_pago}</strong>{item.codigo_promocional?<span className="block text-xs text-cyan-700">{item.codigo_promocional}</span>:null}</td><td className="p-4">{item.numero_operacion||"—"}{item.comprobante?<a href={item.comprobante} target="_blank" rel="noreferrer" className="ml-2 inline-flex items-center gap-1 font-bold text-cyan-700">Ver <ExternalLink className="h-3 w-3"/></a>:null}</td><td className="p-4"><span className={`rounded-full px-3 py-1 text-xs font-bold ${item.estado_pago==="approved"?"bg-emerald-100 text-emerald-800":item.estado_pago==="pending"?"bg-amber-100 text-amber-800":"bg-slate-100"}`}>{paymentLabels[item.estado_pago]}</span></td><td className="p-4">{new Date(item.creado_at).toLocaleDateString("es-UY")}</td><td className="p-4"><div className="flex gap-2"><button onClick={()=>onUpdate(item,"approved")} className="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-3 py-2 font-bold text-white"><Check className="h-4 w-4"/>Aprobar</button><button onClick={()=>onUpdate(item,"rejected")} className="inline-flex items-center gap-1 rounded-lg bg-red-50 px-3 py-2 font-bold text-red-700"><X className="h-4 w-4"/>Rechazar</button></div></td></tr>)}</tbody></table></div>}
