...
type ComercioForm = { nombre: string; direccion: string; telefono: string; descripcion: string; usaWhatsapp: boolean }
type ServicioForm = { nombre: string; categoria: string; descripcion: string; responsable: string; contacto: string; direccion: string; usaWhatsapp: boolean }
type CursoForm = { nombre: string; descripcion: string; responsable: string; contacto: string; usaWhatsapp: boolean }
type InstitucionForm = { nombre: string; descripcion: string; direccion: string; telefono: string; usaWhatsapp: boolean }

const serviceCategories = ["Profesionales", "Alojamientos", "Oficios", "Servicios"]
const initialComercio: ComercioForm = { nombre: "", direccion: "", telefono: "", descripcion: "", usaWhatsapp: true }
const initialServicio: ServicioForm = { nombre: "", categoria: "Profesionales", descripcion: "", responsable: "", contacto: "", direccion: "", usaWhatsapp: true }
const initialCurso: CursoForm = { nombre: "", descripcion: "", responsable: "", contacto: "", usaWhatsapp: true }
const initialInstitucion: InstitucionForm = { nombre: "", descripcion: "", direccion: "", telefono: "", usaWhatsapp: true }
...
function buildOnboardingPayload(params: { type: UserEntityType; email: string; comercio: ComercioForm; servicio: ServicioForm; curso: CursoForm; institucion: InstitucionForm }) {
  if (params.type === "comercio") {
    return { table: "comercios" as const, payload: { nombre: params.comercio.nombre.trim(), direccion: params.comercio.direccion.trim() || null, telefono: params.comercio.telefono.trim() || null, descripcion: params.comercio.descripcion.trim() || null, imagen_url: null, estado: "borrador", owner_email: params.email, usa_whatsapp: params.comercio.usaWhatsapp } }
  }
  if (params.type === "servicio") {
    return { table: "servicios" as const, payload: { nombre: params.servicio.nombre.trim(), categoria: params.servicio.categoria, descripcion: params.servicio.descripcion.trim() || null, responsable: params.servicio.responsable.trim() || null, contacto: params.servicio.contacto.trim() || null, direccion: params.servicio.direccion.trim() || null, imagen: null, estado: "borrador", owner_email: params.email, usa_whatsapp: params.servicio.usaWhatsapp } }
  }
  if (params.type === "curso") {
    return { table: "cursos" as const, payload: { nombre: params.curso.nombre.trim(), descripcion: params.curso.descripcion.trim(), responsable: params.curso.responsable.trim(), contacto: params.curso.contacto.trim(), imagen: null, estado: "borrador", owner_email: params.email, usa_whatsapp: params.curso.usaWhatsapp } }
  }
  return { table: "instituciones" as const, payload: { nombre: params.institucion.nombre.trim(), descripcion: params.institucion.descripcion.trim() || null, direccion: params.institucion.direccion.trim() || null, telefono: params.institucion.telefono.trim() || null, foto: null, estado: "borrador", owner_email: params.email, usa_whatsapp: params.institucion.usaWhatsapp } }
}
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <section className="space-y-6">
            ...
            <EventsSection title="Eventos activos" emptyTitle="No tienes eventos activos" emptyDescription="Cuando un evento se publica, lo veras aqui. Puedes ir cargando nuevos desde tu panel." events={activeEvents} />
            <EventsSection title="Eventos en borrador" emptyTitle="No tienes eventos en borrador" emptyDescription="Los borradores quedan listos para revisar o completar antes de publicarse." events={draftEvents} />
          </section>
...
function EventsSection({ title, emptyTitle, emptyDescription, events }: { title: string; emptyTitle: string; emptyDescription: string; events: UserOwnedEvent[] }) {
  return <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm sm:p-7"><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-2xl font-semibold text-slate-950">{title}</h2><p className="mt-2 text-sm leading-6 text-slate-500">Revisa el estado y el contenido de las publicaciones vinculadas a tu cuenta.</p></div><Link href="/usuarios/eventos/nuevo" className="inline-flex items-center rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-600">Nuevo evento</Link></div>{events.length === 0 ? <div className="mt-6 rounded-[24px] border border-dashed border-slate-200 bg-[linear-gradient(135deg,#f8fbff_0%,#f4faf6_100%)] p-8"><h3 className="text-lg font-semibold text-slate-900">{emptyTitle}</h3><p className="mt-2 max-w-xl text-sm leading-7 text-slate-600">{emptyDescription}</p></div> : <div className="mt-6 grid gap-4 md:grid-cols-2">{events.map((event) => <article key={event.id} className="overflow-hidden rounded-[28px] border border-slate-200 bg-[linear-gradient(180deg,#ffffff_0%,#f8fbff_100%)]">{event.imagen ? <img src={event.imagen} alt={event.titulo} className="h-44 w-full object-cover" /> : <div className="flex h-44 items-center justify-center bg-slate-100"><ImageIcon className="h-8 w-8 text-slate-300" /></div>}<div className="p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-semibold text-slate-900">{event.titulo}</h3><div className="mt-2 inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{event.categoria || "Evento"}</div></div><div className="rounded-full bg-slate-200 px-3 py-1 text-xs font-semibold text-slate-700">{formatEventState(event.estado)}</div></div><div className="mt-4 space-y-2 text-sm text-slate-600"><div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-slate-400" /><span>{new Date(event.fecha).toLocaleDateString("es-UY")}</span></div></div><p className="mt-4 line-clamp-4 text-sm leading-7 text-slate-500">{event.descripcion}</p></div></article>)}</div>}</div>
}

function Field({ label, value, onChange, required }: { label: string; value: string; onChange: (value: string) => void; required?: boolean }) {
  return <div className="space-y-2"><label className="text-sm font-medium text-slate-700">{label}</label><input type="text" value={value} onChange={(event) => onChange(event.target.value)} required={required} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-blue-400" /></div>
}
