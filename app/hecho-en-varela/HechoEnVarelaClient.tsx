...
import { HECHO_EN_VARELA_CATEGORIES, VENTURE_ORIGINS, ventureOriginLabel, formatPrice, whatsappUrl, type ProductoVarela } from "../lib/hechoEnVarela"
...
export function HechoEnVarelaClient({ products }: { products: ProductoVarela[] }) {
  ...
  const ventures = useMemo(() => Array.from(new Map(products.flatMap(product => {
    ...
    return venture && (origin === "Todos" || (venture.origen || "varela") === origin) ? [[venture.id, { id: venture.id, nombre: venture.nombre }]] as const : []
  })).values()).sort((a, b) => a.nombre.localeCompare(b.nombre, "es")), [products, origin])
  const visible = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      return (origin === "Todos" || (product.emprendimientos_varela?.origen || "varela") === origin) && (category === "Todos" || categories.includes(category)) && (ventureId === "Todos" || product.emprendimiento_id === ventureId) && (!term || `${product.nombre} ${venture} ${product.descripcion_breve || ""}`.toLocaleLowerCase("es").includes(term))
    })
  }, [products, query, category, ventureId, origin])
  ...
  return <main className="min-h-screen bg-[#fbf7ef] text-stone-900">
    ...
    <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <fieldset className="mt-4"><legend className="mb-2 text-sm font-bold text-stone-700">Filtrar por origen</legend><div className="flex flex-wrap gap-2">{[{ value: "Todos", label: "Todos los orígenes" }, ...VENTURE_ORIGINS].map(option => <button key={option.value} type="button" aria-pressed={origin === option.value} onClick={() => { setOrigin(option.value); setVentureId("Todos") }} className={`min-h-11 rounded-xl border px-4 py-2 text-sm font-bold transition ${origin === option.value ? "border-amber-800 bg-amber-800 text-white" : "border-stone-300 bg-white text-stone-700 hover:border-amber-700"}`}>{option.label}</button>)}</div></fieldset>
      ...
      {visible.length ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4 xl:grid-cols-5">{visible.map(product => {
        ...
        return <article key={product.id} className="group overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
          ...
          <div className="p-3 sm:p-4"><div className="mb-2 flex items-start justify-between gap-2"><div className="flex flex-wrap gap-1">{(product.categorias?.length ? product.categorias : [product.categoria]).map(item => <span key={item} className="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-bold text-amber-900 sm:text-xs">{item}</span>)}</div>{product.destacado && <span className="shrink-0 text-[10px] font-bold text-orange-600 sm:text-xs">Destacado</span>}</div>
            ...
            {venture && <Link href={`/hecho-en-varela/emprendimiento/${venture.slug}`} className="mt-1 block text-xs font-semibold text-stone-500 hover:text-amber-800 sm:text-sm">{venture.nombre}</Link>}{venture && <p className="mt-1 text-xs font-semibold text-amber-800">{ventureOriginLabel(venture.origen)}</p>}
