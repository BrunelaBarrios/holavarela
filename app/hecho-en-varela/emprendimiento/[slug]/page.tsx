...
import { ventureOriginLabel, formatPrice, type EmprendimientoVarela, type ProductoVarela } from "../../../lib/hechoEnVarela"
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><PublicHeader items={[]} /><main className="min-h-screen bg-[#fbf7ef]"><div className="mx-auto max-w-6xl px-4 py-8 sm:px-6"><Link href="/hecho-en-varela" className="inline-flex items-center gap-1 text-sm font-bold text-stone-600"><ChevronLeft className="h-4 w-4" /> Volver al catálogo</Link>
    <header className="mt-6 rounded-3xl bg-[#efe3cd] p-6 sm:p-9"><div className="flex flex-col gap-5 sm:flex-row sm:items-center">{venture.logo_url ? <img src={venture.logo_url} alt={venture.nombre} className="h-24 w-24 rounded-3xl object-cover" /> : <div className="flex h-24 w-24 items-center justify-center rounded-3xl bg-white"><Store className="h-10 w-10 text-amber-800" /></div>}<div><p className="text-sm font-bold uppercase tracking-widest text-amber-800">{ventureOriginLabel(venture.origen)}</p><h1 className="mt-1 text-4xl font-black">{venture.nombre}</h1><p className="mt-3 max-w-2xl leading-7 text-stone-700">{venture.descripcion}</p></div></div>
